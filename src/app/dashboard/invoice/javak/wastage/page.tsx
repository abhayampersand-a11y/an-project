...
const emptyItem = (): ItemRow => ({ item: "", tr: "1", gross_w: "", touch: "", wastage: "", rate: "" })
...
function JavakWastagePage() {
  ...
  /** Sum of bag total weights whose Tr matches the given value. */
  const bagWeightForTr = React.useCallback(
    (tr: string) => bags.filter((b) => b.tr === tr).reduce((s, b) => s + bagTotal(b), 0),
    [bags],
  )

  const computedItems = items.map((it) => {
    const bag_w = bagWeightForTr(it.tr)
    const net_w = num(it.gross_w) - bag_w
    const fine = Math.round(net_w * (num(it.touch) + num(it.wastage)) / 100)
    const amount = Math.round(net_w * num(it.rate) / 1000)
    return { ...it, bag_w, net_w, fine, amount }
  })
  ...
  const removeBag = (i: number) => setBags((prev) => (prev.length > 1 ? prev.filter((_, idx) => idx !== i) : prev))
              ...
              <div className="flex flex-col gap-2">
                {computedItems.map((it, i) => (
                  <div key={i} className="grid grid-cols-[1.4fr_70px_1fr_1fr_1fr_1fr_1fr_1fr_1fr_1fr_56px] items-center gap-2">
                    ...
                    <Select
                      value={it.tr}
                      onValueChange={(v) => { if (v) setItem(i, { tr: v }) }}
                    ...
                    <Input type="number" value={it.gross_w} onChange={(e) => setItem(i, { gross_w: e.target.value })} />
                    <Input value={it.bag_w || ""} readOnly className="bg-muted" />
                    <Input value={it.net_w || ""} readOnly className="bg-muted" />
