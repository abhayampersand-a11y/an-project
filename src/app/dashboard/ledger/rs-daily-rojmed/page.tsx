...
export default function RsDailyRojmedPage() {
  ...
  async function handleCheck() {
    setLoading(true)
    try {
      const res = await fetch(`/api/ledger/rs-daily-rojmed?date=${date}`)
  ...
  return (
    <div className="flex flex-1 flex-col gap-4 p-4 md:gap-6 md:p-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h1 className="text-xl font-semibold">DAILY RS ROJMED</h1>
        <p className="text-sm text-muted-foreground">Om Casting &rsaquo; Daily Rs Rojmed</p>
      </div>
      ...
      {/* statement */}
      {stmt && (
        <div id="dailyrs-statement" className="rounded-lg border bg-card p-4 md:p-6">
          <style>{`
            @media print {
              body * { visibility: hidden !important; }
              #dailyrs-statement, #dailyrs-statement * { visibility: visible !important; }
              #dailyrs-statement { position: absolute; top: 0; left: 0; width: 100%; }
            }
          `}</style>
