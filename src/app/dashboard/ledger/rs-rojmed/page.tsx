...
export default function RsRojmedPage() {
  ...
  async function handleCheck() {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/ledger/rs-rojmed?party=${encodeURIComponent(party)}&from=${from}&to=${to}`)
  ...
  return (
    <div className="flex flex-1 flex-col gap-4 p-4 md:gap-6 md:p-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h1 className="text-xl font-semibold">RS ROJMED</h1>
        <p className="text-sm text-muted-foreground">Om Casting &rsaquo; Rs Rojmed</p>
      </div>
      ...
      {/* statement */}
      {stmt && (
        <div id="rs-statement" className="rounded-lg border bg-card p-4 md:p-6">
          <style>{`
            @media print {
              body * { visibility: hidden !important; }
              #rs-statement, #rs-statement * { visibility: visible !important; }
              #rs-statement { position: absolute; top: 0; left: 0; width: 100%; }
            }
          `}</style>
          ...
          <div className="grid gap-8 lg:grid-cols-2">
            <Particulars title="Credit Particulars" valueLabel="Rs" entries={stmt.credit} total={stmt.creditTotal} />
            <div className="flex flex-col gap-3">
              <Particulars title="Debit Particulars" valueLabel="Labour Rs" entries={stmt.debit} total={stmt.debitTotal} />
          ...
          {entries.length === 0 ? (
            ...
            entries.map((e, i) => (
              <tr key={i} className="border-b border-dashed">
                ...
                <td className="px-2 py-1.5 text-right">{inr(e.rs)}</td>
